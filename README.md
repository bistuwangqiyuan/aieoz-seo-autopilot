...
4. **跨页检查**（单页审计看不到的问题）：canonical 是否自指、中英 hreflang 是否成对且有 `x-default`、
   sitemap 内 URL 轮转抽样可达性。canonical / hreflang 问题**逐 URL 跨轮累计**成全站待修清单，
   页面复审通过即自动移出——只报本轮抽样值会让轮转一过就显示为已修好，等于给官网团队错误信号
...
5. **事实一致性巡检**：优先复核「未按现行规则集检过」的文章，其余按「最久未检优先」轮转；
   命中违规规则时由 AI 依据已核实资料重写并回写平台。规则集带版本号，
   新增规则会让全部存量文章自动重回队列（见下文）；重写后仍不合规的文章会在面板列出，而不是被默认放过
...
```bash
npx tsx scripts/test-integrity.ts                             # 一致性规则：拦截 34 条编造/错配 + 2 条虚假标题 / 放行 21 条事实 + 3 类诚实标题
...
| **标题承诺未实测型号的实测数据** | 标题不含数字，句级校验抓不到；而对检索引擎标题是权重最高的信号 |
...
上述规则上线并自动修好 6 篇文章后，抽查线上正文发现一篇：正文已把全部数字正确归给 FX100，
标题却仍是「Comprehensive Benchmarks for Mingxin FX300」—— **每句话都对，标题却把结论说错了**。
标题里没有数字，逐句归属校验按构造就看不见它；而对检索引擎来说，标题是全页权重最高的信号，
只改正文等于没改。

因此增加了一条文档级规则：标题点名 FX200/FX300/FX400 且承诺提供实测数据（benchmarks / test results /
performance figures…）即判违规，修复时允许 AI 一并改写标题并回写平台。
被放行的边界同样写进了测试：比较这些型号、讨论这些型号、教读者「如何自测 FX300」都合法 ——
一条让 FX200/FX300 变成不可提及的规则，是用一种失真换另一种失真。

另外，诚实的修复常常比它替换掉的虚假版本更短（实测那篇从 1200+ 词降到 978 词）。
修复路径现在同样受 1200 词标准约束，但**顺序是明确的：诚实优先于字数**——
扩写失败绝不能变成「放弃修复」，否则虚假版本会继续留在公网上。
...
「张冠李戴」规则上线时，线上已有 5 篇文章正好犯这个错，而巡检按「最久未检优先」轮转，
要一天才会轮到它们 —— 实际是靠人工跑脚本发现的，而这恰恰是本系统唯一不允许依赖的东西。

因此每篇文章会记录它是**按哪一版规则集**判定为合规的（`integrityRulesVersion`，
由全部规则正则 + 已核实产品资料共同哈希得出）。只要新增规则、修改正则，
或更新已核实资料（数值白名单随之改变），版本号即变化，**全部存量文章自动回到待复核队列队首**。
检验只耗 CPU、只有命中违规才耗 AI 调用，所以规则变更后单轮扫描 40 篇（远超能修复的数量），
一次就摸清欠账规模；修复受时间预算限制，未修完的文章不打版本戳，下一轮继续。
`/api/status` 的 `effect.integrity.staleForRules` 与面板「事实一致性自动巡检」卡片会显示欠账篇数，
规则刚更新时非零属正常，应当自动回零。
